...
def calculate_position_size(api_key, default_size, smart_sizing):
    ...
    # Try to get balance from portfolio (balance_usdc for real trading, or from agents/me)
    balance = portfolio.get("balance_usdc", 0)
    
    # If no USDC balance, try to get SIM balance from agents/me
    if balance <= 0:
        agent_info = sdk_request(api_key, "GET", "/api/sdk/agents/me")
        if "error" not in agent_info:
            balance = agent_info.get("balance", 0)
...
def fetch_markets(api_key, market_filter=""):
    ...
    # Filter out crypto markets
    crypto_keywords = ['bitcoin', 'btc', 'ethereum', 'eth', 'solana', 'sol', 'xrp', 'crypto', 'coin', 'token']
    markets = [m for m in markets if not any(kw in m.get('question', '').lower() for kw in crypto_keywords)]
...
def run_mert_strategy(dry_run=True, positions_only=False, show_config=False,
    ...
    if smart_sizing:
        ...
        if portfolio:
            balance = portfolio.get('balance_usdc', 0)
            # If no USDC, try to get SIM balance
            if balance <= 0:
                agent_info = sdk_request(api_key, "GET", "/api/sdk/agents/me")
                if "error" not in agent_info:
                    balance = agent_info.get("balance", 0)
