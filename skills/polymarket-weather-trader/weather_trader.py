...
# Trade Logger integration - 记录到 prediction-log.js
try:
...
CONFIG_SCHEMA = {
    ...
    "max_market_exposure_usd": {"env": "SIMMER_WEATHER_MAX_MARKET_EXPOSURE", "default": 30.00, "type": float},
...
MAX_MARKET_EXPOSURE_USD = _config["max_market_exposure_usd"]
        ...
        if current_price >= EXIT_THRESHOLD:
            ...
            if dry_run:
                ...
                if result.get("success"):
                    ...
                    # Note: 卖出时无法自动关联到 prediction-log.js 的原始交易ID
                    # 需要手动通过市场名称匹配后结算
                    # TODO: 实现自动ID映射机制
                    if TRADE_LOGGER_AVAILABLE:
                        print(f"     💡 Tip: 手动结算交易 - ./prediction-log.js open 查看持仓")
...
def run_weather_strategy(dry_run: bool = True, positions_only: bool = False,
    ...
    log(f"  Max market exposure: ${MAX_MARKET_EXPOSURE_USD:.2f}")
        ...
        if price < ENTRY_THRESHOLD:
            ...
            for pos in positions:
                if pos.get('market_id') == market_id:
                    ...
                    current_exposure = (shares_yes * current_price) + (shares_no * (1 - current_price))
                    break
            
            if current_exposure >= MAX_MARKET_EXPOSURE_USD:
                log(f"  ⏸️  Market exposure ${current_exposure:.2f} >= limit ${MAX_MARKET_EXPOSURE_USD:.2f} - skip")
                continue
            
            # Adjust position size to not exceed market limit
            remaining_capacity = MAX_MARKET_EXPOSURE_USD - current_exposure
            if position_size > remaining_capacity:
                position_size = remaining_capacity
                log(f"  💡 Adjusted position to ${position_size:.2f} (market limit)")
            ...
            if dry_run:
                ...
                if result.get("success"):
                    ...
                    # Log to prediction-log.js
                    if TRADE_LOGGER_AVAILABLE:
                        reason = f"NOAA forecasts {forecast_temp}°F for {location} on {date_str}, bucket '{outcome_name}'"
                        log_buy(
                            platform="Simmer",
                            market=question,
                            option="Yes",
