...
// 检查是否为双孕线
function isDoubleHarami(k1, k2, k3, minBodySize = 150) {
    const b1 = getBody(k1);
    const b2 = getBody(k2);
    const b3 = getBody(k3);
...
function validateSignal(data, index, type) {
    ...
    if (future.length < 3) return null; // 数据不足
    ...
    if (type === '看涨双孕线') {
        // 期望上涨
        if (upMove > 1.5) { // 上涨超过1.5%
            result = 'win';
            profit = upMove;
        } else if (downMove > 1.5) { // 下跌超过1.5%
            result = 'loss';
            profit = -downMove;
        }
    } else {
        // 期望下跌
        if (downMove > 1.5) { // 下跌超过1.5%
            result = 'win';
            profit = downMove;
        } else if (upMove > 1.5) { // 上涨超过1.5%
            result = 'loss';
            profit = -upMove;
        }
    }
...
async function backtest() {
    console.log('开始回测 2025年至今的 4小时双孕线形态...\n');
    ...
    try {
        ...
        while (currentTime < now) {
            ...
            const ohlcv = await exchange.fetchOHLCV('BTC/USDT', '4h', currentTime, 1000);
            ...
            currentTime = ohlcv[ohlcv.length - 1][0] + 4 * 60 * 60 * 1000;
            
            // 避免请求过快
            await new Promise(resolve => setTimeout(resolve, 500));
        }
        
        console.log(`\n共获取 ${allData.length} 根4小时K线\n`);
